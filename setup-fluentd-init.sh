...
USER=fluent
GROUP=fluent
PIDFILE=/var/run/${NAME}/${PNAME}.pid
LOGFILE=/var/log/${NAME}/${PNAME}.log
DAEMON_ARGS="--config /etc/${NAME}/${NAME}.conf --daemon ${PIDFILE} --log ${LOGFILE} --user ${USER} --group ${GROUP}"
...
mkdir -p /etc/fluent/ /var/run/fluent/ /var/log/fluent/ /var/lib/fluent/
chown -R fluent:fluent /etc/fluent/ /var/run/fluent/ /var/log/fluent/ /var/lib/fluent/ /etc/init.d/fluentd
sudo -u fluent fluentd --setup /etc/fluent/
chmod 755 /etc/init.d/fluentd
